...
setwd("")
##### Import data ps5.rds
permafrost <- readRDS("ps5.rds")
# Convert ASV table to .csv format 
my_ASVs <- write_phyloseq(permafrost, type = "OTU", path = getwd())

#for phylogenetic signal analysis:
ASVs = as(otu_table(permafrost), "matrix")
# Coerce to data.frame
my_ASVs = as.data.frame(ASVs)
write.csv(my_ASVs, "~my_ASVs_ps.csv", row.names=TRUE, col.names=TRUE)

#for assembly analysis (needs to be transposed):
ASVs = as(otu_table(permafrost), "matrix")
# Coerce to data.frame
my_ASVs = as.data.frame(ASVs)
my_ASVs = t(my_ASVs)
write.csv(my_ASVs, "~my_ASVs_a.csv", row.names=TRUE, col.names=TRUE)


# Convert tree to .nwk format
tree = phy_tree(permafrost)
ape::write.tree(tree, "~tree.nwk")


#### The remaining code was moved to the HPC to compute
...
## Please cite the original paper if using this script! I have merely added notes for clarification. Original code can be found on Stegen GitHub.
...
otu = read.csv("my_ASVs_a.csv",header=T,row.names=1);
...
## Read in ASV table 
## This table was generated in qiime2, exported to a .biom file, then converted to a .csv file (code not shown)
asv = read.csv("my_ASVs_a.csv",header=T,row.names=1);
asv = t(asv)
